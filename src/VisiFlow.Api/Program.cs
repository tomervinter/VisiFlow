...
// Per-channel daily capacity override (see ChannelCapacity.cs / VisitPlanGenerator.PrimaryChannelOf) -
// one row per sales channel currently present anywhere in the company's customer data, lazily seeded
// (from the company-wide VisitPlanWeights default) the first time this is requested. Re-seeds any
// NEWLY-appeared channel on every call too (not just when the table is completely empty), so a channel
// that shows up for the first time in a later month's customer upload still gets a row automatically.
