...
public class VisitPlanGeneratorTests : IDisposable
{
    ...
    // Regression coverage for a deliberate product decision: a plan is built strictly one month at a
    // time and must never borrow a date from the month before it, even for a customer whose
    // distribution day falls on the 1st/2nd of the month (so "2 days before delivery" would otherwise
    // land in the previous month). March 1, 2026 is a Sunday (and, like February 2026, has no
    // IsraeliHolidays entries - see this file's header comment), so a Sunday-only distribution customer
    // exercises exactly that boundary without depending on real holiday data.
    [Fact]
    public async Task GenerateAsync_DistributionOccurrenceOnFirstOfMonth_StaysWithinMonthAndFallsBackToDeliveryDay()
    {
        var company = await SeedCompanyAsync();
        ...
        // "2 days before" (Feb 27) and "1 day before" (Feb 28) both fall in February, outside this
        // month's plan - never a legal target, so the visit falls back to the delivery day itself
        // (Sunday March 1), still squarely within PlanYear/PlanMonth.
