...
/// (VisitPlanWeights.FullDayCapacity/HalfDayCapacity), but ONLY when the admin has actually opted into
/// per-channel mode (see VisitPlanWeights.UseChannelCapacity) - otherwise these rows are configured but
/// dormant, and the generator uses the flat company-wide default for every agent regardless. One row
/// per (company, channel), lazily seeded from that same company-wide default the first time a company's
/// list is requested (see GET /api/visitplan/channelcapacity) for every channel currently present in
/// its customer data.</summary>
