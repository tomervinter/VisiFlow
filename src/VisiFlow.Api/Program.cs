...
app.MapGet("/api/cities", async (int companyId, VisiFlowDbContext db) =>
    (await db.Customers.Where(c => c.CompanyId == companyId && c.City != null).Select(c => c.City!).Distinct().ToListAsync())
...
app.MapPost("/api/nonvisitreasons/{id:int}/active", async (int id, SetNonVisitReasonActiveRequest req, VisiFlowDbContext db) =>
{
    var reason = await db.NonVisitReasons.FindAsync(id);
    if (reason == null) return Results.NotFound();
...
app.MapGet("/api/citygroups", async (int companyId, VisiFlowDbContext db) =>
    (await db.CityGroups.Where(g => g.CompanyId == companyId).OrderBy(g => g.Name).ToListAsync())
        .Select(CityGroupDto.From)
).RequireAuthorization();

app.MapPost("/api/citygroups", async (SaveCityGroupRequest req, VisiFlowDbContext db) =>
{
    if (await db.Companies.FindAsync(req.CompanyId) == null) return Results.BadRequest("החברה שנבחרה אינה קיימת");
...
app.MapPost("/api/citygroups/{id:int}", async (int id, SaveCityGroupRequest req, VisiFlowDbContext db) =>
{
...
app.MapDelete("/api/citygroups/{id:int}", async (int id, VisiFlowDbContext db) =>
{
    var group = await db.CityGroups.FindAsync(id);
    if (group == null) return Results.NotFound();
